...
# --- Credential files ---
CRED_FILE=${GITHUB_CRED_FILE:-"$SCRIPT_DIR/creds/jenkins-creds.json"}

# --- DSL and seed job ---
DSL_FILE="$SCRIPT_DIR/jobs/jobs.groovy"
...
if [[ -n "$JENKINS_USER_INPUT" && -n "$JENKINS_TOKEN_INPUT" ]]; then
  ...
  echo "✅ Using Jenkins user/token from creds/jenkins-creds.json"
else
  JENKINS_USER="admin"
